...
let snapshot = null;
let ui = { activeTab:'personagem', error:'', stage:null, onboardingShownFor:null, hintsSeenCount:0, murdererOverlayDismissed:false };
...
const TUTORIAL = [
  ...
  ["Seu personagem", "Você recebe um objetivo, um segredo, relações com os outros e um álibi público — a história que seu personagem já conta sobre onde esteve. Pode mentir em voz alta, mas o aplicativo guarda o que já foi declarado."],
  ["Investigação", "Cada local revela pistas aos poucos. As ações de investigação são limitadas e compartilhadas pelo grupo — usem com cuidado. Algumas pistas só aparecem depois de investigarem outro local primeiro."],
  ["Cifras 🔐", "Algumas evidências chegam codificadas. O texto aparece cifrado — toquem em 'Decifrar' para revelar. Não custa ação extra, mas obriga o grupo a prestar atenção no que está escrito."],
  ["Pistas deterioráveis ⌛", "Certas evidências somem se o grupo demorar demais. O texto fica em laranja quando há risco. Priorizem locais com pistas urgentes."],
  ["Testemunhas limitadas 👁️", "Alguns personagens têm uma limitação (sem óculos, bêbado, daltônico, criança) que distorce o que viram. O relato é sincero — mas pode estar errado. Cruzem com outras pistas."],
  ["Pista plantada 🎭", "O culpado pode gastar uma ação para plantar uma evidência falsa num local. Revelada no final — mas vocês precisam perceber antes que estão sendo enganados."],
  ["Acareação ⚡", "Na aba Locais, quando há uma acareação disponível, o grupo pode gastar 1 ação para confrontar dois suspeitos com contradições — um deles vai revelar algo inédito que não falaria sozinho."],
  ["Declarações", "A linha do tempo já começa preenchida com o álibi de cada um. Na aba Pessoas, mudem o que quiserem, em voz alta, e atualizem os campos."],
  ...
  ["Votação", "No final, cada jogador acusa alguém e explica o motivo. O app revela a verdade só depois de todos votarem — inclusive quem mentiu em cada horário, e se havia pistas plantadas."]
];
...
function renderTitleScreen(){
  ...
  wrap.innerHTML = `
    ...
    <p class="small center" style="margin-top:4px;opacity:0.4;">v${VERSION}</p>
  `;
...
function render(){
  ...
  bar.innerHTML = `<span class="tag">Dossiê Criminal</span><span class="num">CASO ${num} / ${snapshot.caseInfo.title.toUpperCase()}</span><span class="help" id="helpBtn">?</span><span class="version-badge">v${VERSION}</span>`;
  app.appendChild(bar);
...
function characterCardHTML(c, opts){
  ...
  `;
  // mecanica 3: badge de limitação de testemunha
  if(c.witnessLimitation){
    const icons = {sem_oculos:'👓',alcoolizado:'🍺',daltonico:'🎨',crianca:'🧒',nervoso:'😰',distancia:'🔭',medicada:'💊',panico:'😱'};
    const icon = icons[c.witnessLimitation.type] || '👁️';
    html += `<div class="witness-limit-box">${icon} <b>Atenção — testemunha com limitação:</b> ${c.witnessLimitation.description}</div>`;
  }
  if(murderer){
    html += `
...
function renderMurdererOverlay(c){
  const overlay = document.createElement('div');
  overlay.id = 'murderer-overlay';
  overlay.innerHTML = `
    <div class="mrd-redjohn" aria-hidden="true">
      <svg viewBox="0 0 320 340" xmlns="http://www.w3.org/2000/svg">
        <!-- rosto — forma imperfeita, como pincelada -->
        <ellipse cx="160" cy="155" rx="105" ry="118" fill="none" stroke="rgba(180,20,20,0.55)" stroke-width="6" stroke-linecap="round"
          pathLength="100" stroke-dasharray="97 3" style="filter:url(#rough)"/>
        <!-- olho esquerdo -->
        <ellipse cx="120" cy="128" rx="14" ry="11" fill="none" stroke="rgba(200,25,25,0.65)" stroke-width="5" stroke-linecap="round"
          pathLength="100" stroke-dasharray="95 5" style="filter:url(#rough)"/>
        <!-- pupila esquerda -->
        <circle cx="120" cy="128" r="5" fill="rgba(200,25,25,0.7)" style="filter:url(#rough)"/>
        <!-- olho direito -->
        <ellipse cx="200" cy="128" rx="14" ry="11" fill="none" stroke="rgba(200,25,25,0.65)" stroke-width="5" stroke-linecap="round"
          pathLength="100" stroke-dasharray="95 5" style="filter:url(#rough)"/>
        <!-- pupila direita -->
        <circle cx="200" cy="128" r="5" fill="rgba(200,25,25,0.7)" style="filter:url(#rough)"/>
        <!-- sorriso largo, curvado, irregular — traço de pincel -->
        <path d="M 90 190 Q 100 230 160 242 Q 220 230 232 190" fill="none" stroke="rgba(200,25,25,0.6)" stroke-width="7"
          stroke-linecap="round" pathLength="100" stroke-dasharray="96 4" style="filter:url(#rough)"/>
        <!-- cantos do sorriso, levemente escorridos -->
        <path d="M 88 188 Q 80 200 85 210" fill="none" stroke="rgba(200,25,25,0.45)" stroke-width="5" stroke-linecap="round"/>
        <path d="M 233 188 Q 241 200 237 210" fill="none" stroke="rgba(200,25,25,0.45)" stroke-width="5" stroke-linecap="round"/>
        <defs>
          <feTurbulence type="fractalNoise" baseFrequency="0.05" numOctaves="2" result="noise" id="turbulence"/>
          <feDisplacementMap in="SourceGraphic" in2="noise" scale="3" xChannelSelector="R" yChannelSelector="G" result="displaced"/>
          <filter id="rough" x="-5%" y="-5%" width="110%" height="110%">
            <feTurbulence type="fractalNoise" baseFrequency="0.04 0.07" numOctaves="3" result="n"/>
    ...
    <div class="mrd-content">
      <div class="mrd-eyebrow">DOSSIÊ CRIMINAL — CONFIDENCIAL</div>
      <div class="mrd-you-are">Você é</div>
      <div class="mrd-role">O CULPADO</div>
      <div class="mrd-name">${c.name}</div>
      <div class="mrd-rule"></div>
      <p class="mrd-motive">${c.segredo}</p>
      <p class="mrd-hint">Só você sabe disso.<br>Não revele a ninguém.</p>
      <button class="mrd-btn" id="mrdContinue">Entendido — ver minha ficha completa</button>
    </div>
  `;
  document.body.appendChild(overlay);
  // pequeno delay pra animação de entrada
  requestAnimationFrame(()=>{ requestAnimationFrame(()=>{ overlay.classList.add('mrd-visible'); }); });
  overlay.querySelector('#mrdContinue').onclick = (e)=>{
    e.stopPropagation();
    overlay.classList.add('mrd-exit');
    setTimeout(()=>{
      overlay.remove();
      ui.murdererOverlayDismissed = true;
      render();
    }, 600);
  };
}
...
function renderReveal(main){
  ...
  if(!me.ready){
    ...
    // se é o culpado e ainda não viu o overlay, mostra a tela fullscreen primeiro
    if(c.isMurderer && !ui.murdererOverlayDismissed){
      renderMurdererOverlay(c);
      return;
    }
    main.innerHTML += `
...
function tabLocais(body){
  const inv = snapshot.investigation;
  const me = snapshot.me;
  const myChar = me ? me.character : null;
  const isMurderer = myChar && myChar.isMurderer;
  ...
  // mecanica 4: botao de pista plantada (só pro culpado)
  if(isMurderer && myChar.plantAction && !inv.plantedClueUsed){
    const pb = document.createElement('div');
    pb.className = 'card murderer-card';
    pb.style.marginBottom = '14px';
    pb.innerHTML = `
      <div class="murderer-banner">🎭 Ação especial — apenas você vê isso</div>
      <p>Você pode plantar uma falsa pista em <b>${myChar.plantAction.targetName}</b> para desviar as suspeitas.</p>
      <button class="btn primary" id="plantBtn" ${inv.actionsLeft<=0?'disabled':''}>Plantar pista falsa (−1 ação)</button>
    `;
    body.appendChild(pb);
    pb.querySelector('#plantBtn').onclick = ()=> post('/api/plant-clue', {playerId});
  }
  ...
  inv.evidenceTexts.forEach(loc=>{
    ...
    // evidencias reveladas
    loc.revealed.forEach((ev,i)=>{
      if(ev.cipher && !ev.cipherRevealed){
        // mecanica 1: cifra ainda não revelada
        const cipherKey = `${loc.id}-${i}`;
        html += `<div class="evidence-block reveal-pop cipher-block">
          <span class="tag">${ev.tag} 🔐 Cifrado</span>
          <p class="cipher-encoded">${ev.cipher.encoded}</p>
          <button class="btn ghost small" data-cipher="${cipherKey}" data-loc="${loc.id}" data-idx="${i}">Decifrar</button>
        </div>`;
      } else {
    ...
    // pistas plantadas neste local (mecanica 4)
    (loc.planted||[]).forEach(p=>{
      html += `<div class="evidence-block reveal-pop"><span class="tag">${p.evidence.tag}</span><p style="margin-top:4px;">${p.evidence.text}</p></div>`;
    });

    // próxima evidência
    if(revealedCount < meta.evidenceCount){
      if(loc.nextExpired){
        html += `<p class="small" style="color:var(--carmine)">⌛ Esta evidência se deteriorou — não foi investigada a tempo.</p>`;
      } else if(loc.lockedOn){
    ...
    } else if(!loc.planted || loc.planted.length===0){
  ...
  body.querySelectorAll('button[data-loc]').forEach(btn=>{
    btn.onclick = ()=>{ btn.classList.add('btn-press'); post('/api/investigate', { locationId: btn.dataset.loc }); };
  });

  // mecanica 1: botão de decifrar
  body.querySelectorAll('button[data-cipher]').forEach(btn=>{
    btn.onclick = ()=>{
      post('/api/reveal-cipher', { locationId: btn.dataset.loc, evidenceIndex: parseInt(btn.dataset.idx) });
    };
  });

  // mecanica 5: acareações disponíveis
  const availConfront = inv.availableConfront || [];
  if(availConfront.length > 0){
    const cf = document.createElement('div');
    cf.className = 'card';
    cf.innerHTML = `<div class="meta">Acareações disponíveis</div>
      <p class="small">Gasta 1 ação — força dois suspeitos a se confrontarem, revelando um detalhe que nenhum dos dois quis contar.</p>`;
    availConfront.forEach(c=>{
      if(c.revealed) return;
      const nameA = snapshot.caseInfo.characters.find(x=>x.id===c.charIds[0])?.name || c.charIds[0];
      const nameB = snapshot.caseInfo.characters.find(x=>x.id===c.charIds[1])?.name || c.charIds[1];
      const btn = document.createElement('button');
      btn.className = 'btn ghost';
      btn.style.marginTop = '8px';
      btn.textContent = `Confrontar ${nameA} × ${nameB} (−1 ação)`;
      btn.disabled = inv.actionsLeft <= 0;
      btn.onclick = ()=> post('/api/confront', { charIdA: c.charIds[0], charIdB: c.charIds[1] });
      cf.appendChild(btn);
    });
    // mostra resultados de acareações já feitas
    Object.entries(inv.confrontClues || {}).forEach(([key, val])=>{
      if(!val.revealed) return;
      const box = document.createElement('div');
      box.className = 'hint-block reveal-pop';
      box.innerHTML = `<span class="tag">Acareação revelada</span>${val.clue.text}`;
      cf.appendChild(box);
    });
    body.appendChild(cf);
  }
...
function renderResult(main){
  ...
  main.innerHTML += `
    ...
    ${r.plantedClues && r.plantedClues.length ? `
    <div class="card fade-in">
      <div class="meta" style="color:var(--carmine)">Pistas falsas plantadas</div>
      <p class="small">O culpado injetou evidências fabricadas para desviar a investigação.</p>
      ${r.plantedClues.map(p=>`<div class="evidence-block"><span class="tag" style="color:var(--carmine)">${p.evidence.tag} — fabricada por ${p.planterName}</span><p style="margin-top:4px;">${p.evidence.text}</p></div>`).join('')}
    </div>` : ''}
    <div class="btn-row">
