...
function freshGame(caseId) {
  return {
    ...
    totalActionsSpent: 0,   // mecanica 2: conta ações gastas pra pistas deterioráveis
    discoveredCiphers: {},  // locId-evidenceIndex -> true: cipher já revelada (mecanica 1)
    plantedClues: [],       // mecanica 4: {locId, evidence:{tag,text}, planterId, planted:true}
    murdererPlantUsed: false, // mecanica 4
    confrontClues: {},      // mecanica 5: "charA-charB" -> {revealed:bool}
    publicFacts: [],
...
function buildSnapshot(playerId) {
  ...
  if (["investigation", "voting", "result"].includes(game.phase)) {
    ...
    snapshot.investigation = {
      ...
      evidenceTexts: cs.locations.map(l => {
        ...
        // mecanica 2: pistas deterioráveis — verifica se próxima pista expirou
        const nextExpired = nextItem && nextItem.expiresAfterActions !== undefined && game.totalActionsSpent >= nextItem.expiresAfterActions;
        // mecanica 1: decifrar — mostra cipher encriptada se ainda não foi revelada
        const revealed = (l.evidence || []).slice(0, already).map((ev, i) => {
          const isCiphered = !!ev.cipher;
          const cipherKey = `${l.id}-${i}`;
          const cipherRevealed = !isCiphered || !!game.discoveredCiphers[cipherKey];
          return {
            tag: ev.tag,
            text: cipherRevealed ? ev.text : null,  // não manda o texto até o jogador decifrar
            cipher: ev.cipher || null,
            cipherRevealed
          };
        });
        // pistas plantadas neste local (mecanica 4)
        const planted = game.plantedClues.filter(p => p.locId === l.id);
        return { id: l.id, name: l.name, revealed, planted, lockedOn, nextExpired };
      }),
      discoveredCiphers: game.discoveredCiphers,
      plantedClueUsed: game.murdererPlantUsed,
      confrontClues: game.confrontClues,
      totalActionsSpent: game.totalActionsSpent,
      availableConfront: (cs.confrontClues || []).map(c => ({
        charIds: c.charIds,
        locationId: c.locationId,
        revealed: !!(game.confrontClues[c.charIds.slice().sort().join("-")])
      })),
  ...
  if (game.phase === "result") {
    ...
    snapshot.result = {
      ...
      plantedClues: game.plantedClues.map(p => ({ ...p, planterName: charById(p.planterId)?.name })),
      timelineReveal
    };
  }
...
const api = {
  ...
  // debug: força um jogador específico a receber o personagem assassino (só pra testes)
  "/api/debug-set-murderer": async (req, res, body) => {
    if (game.phase !== "reveal") return sendJSON(res, 409, { error: "Só na fase de revelação." });
    const p = findPlayer(body.playerId);
    if (!p) return sendJSON(res, 404, { error: "Jogador não encontrado." });
    const murderer = effChars().find(c => c.isMurderer);
    if (!murderer) return sendJSON(res, 400, { error: "Nenhum assassino nesta partida." });
    // troca o personagem deste jogador pelo assassino (e troca o antigo assassino pro personagem que era dele)
    const oldCharId = p.characterId;
    const murdererPlayer = game.players.find(pl => pl.characterId === murderer.id);
    if (murdererPlayer) murdererPlayer.characterId = oldCharId;
    p.characterId = murderer.id;
    broadcast();
    sendJSON(res, 200, {});
  },

  // mecanica 1: revelar uma cipher (action-free, mas irreversível)
  "/api/reveal-cipher": async (req, res, body) => {    if (game.phase !== "investigation") return sendJSON(res, 409, { error: "Fora da fase de investigação." });
    const { locationId, evidenceIndex } = body;
    game.discoveredCiphers[`${locationId}-${evidenceIndex}`] = true;
    broadcast();
    sendJSON(res, 200, {});
  },

  // mecanica 4: assassino planta pista falsa num local
  "/api/plant-clue": async (req, res, body) => {
    if (game.phase !== "investigation") return sendJSON(res, 409, { error: "Fora da fase de investigação." });
    const me = findPlayer(body.playerId);
    if (!me) return sendJSON(res, 404, { error: "Jogador não encontrado." });
    const myChar = charById(me.characterId);
    if (!myChar || !myChar.isMurderer) return sendJSON(res, 403, { error: "Só o culpado pode usar esta ação." });
    if (game.murdererPlantUsed) return sendJSON(res, 409, { error: "Ação de pista plantada já usada." });
    if (game.actionsLeft <= 0) return sendJSON(res, 400, { error: "Sem ações restantes." });
    const plantOpts = myChar.plantAction;
    if (!plantOpts) return sendJSON(res, 400, { error: "Este personagem não tem pista para plantar." });
    game.plantedClues.push({ locId: plantOpts.locationId, evidence: plantOpts.evidence, planterId: me.characterId, planted: true });
    game.murdererPlantUsed = true;
    game.actionsLeft--;
    game.totalActionsSpent++;
    broadcast();
    sendJSON(res, 200, {});
  ...
  // mecanica 5: acareação entre dois suspeitos
  "/api/confront": async (req, res, body) => {
    if (game.phase !== "investigation") return sendJSON(res, 409, { error: "Fora da fase de investigação." });
    if (game.actionsLeft <= 0) return sendJSON(res, 400, { error: "Sem ações restantes." });
    const { charIdA, charIdB } = body;
    const key = [charIdA, charIdB].sort().join("-");
    if (game.confrontClues[key]) return sendJSON(res, 400, { error: "Esta acareação já foi feita." });
    const cs = currentCase();
    const confrontDef = (cs.confrontClues || []).find(c => c.charIds.slice().sort().join("-") === key);
    if (!confrontDef) return sendJSON(res, 404, { error: "Não há acareação disponível entre esses dois." });
    game.confrontClues[key] = { revealed: true, clue: confrontDef.clue };
    game.actionsLeft--;
    game.totalActionsSpent++;
    broadcast();
    sendJSON(res, 200, {});
